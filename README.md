...
Some Microsoft Graph APIs return all the requested entities by default, and in that case the ```@odata.nextLink``` element is not provided. The absense of this element tells you that the ```value``` element contains all entities in a single page.
...
The same API can also return paginated responses, if the result set is larger than the page size. The following example uses the ```$top``` query string parameter to return only the first 10 items from the same set:
...
As a best practice, your code should allow for the fact that ```@odata.nextLink``` might be missing, in which case there is no pagination to be handled. The following generator sample shows an example of this.
